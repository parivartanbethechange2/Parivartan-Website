...
export default function Events() {
  ...
  const rsvp = async (e) => {
    ...
    try {
      ...
      toast.success(data.is_rsvped ? `You're going to ${e.title}` : "RSVP cancelled");
      await load();
    } catch {
    ...
    <>
        ...
        <div className="mx-auto flex max-w-[1400px] flex-wrap items-center gap-3 px-6 py-6">
          {kinds.map((k) => (
            <button
              key={k}
              onClick={() => setKind(k)}
              data-testid={`event-filter-${k.replace(/\W+/g, "-").toLowerCase()}`}
              className={`border px-5 py-2.5 text-xs transition-colors duration-300 ${
                kind === k ? "border-forest bg-forest text-sand" : "border-forest/25 text-ink/60 hover:border-forest/60"
              }`}
            >
              {k}
            </button>
      ...
      <section className="bg-sand">
        <div className="mx-auto max-w-[1400px] px-6 py-24">
                    ...
                    <StaggerItem key={e.id}>
                      ...
                      >
                        <div className="flex flex-col items-center justify-center border-b border-line bg-sage/50 py-6 md:border-b-0 md:border-r">
                          <span className="serif text-5xl leading-none tracking-tight text-forest">{d.getDate()}</span>
                        ...
                        <div className="p-7 md:p-9">
                          <span className="overline border border-line px-3 py-1 text-ink/55">{e.kind}</span>
                          <h3 className="mt-5 serif text-2xl leading-snug tracking-tight md:text-3xl">{e.title}</h3>
                          <p className="mt-3 text-sm leading-relaxed text-ink/65">{e.description}</p>
                          <div className="mt-6 flex flex-wrap gap-x-7 gap-y-2 text-xs text-ink/50">
                            ...
                            <span className="flex items-center gap-2">
                              <Users size={13} /> {e.rsvp_count} / {e.capacity} confirmed
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="flex flex-col justify-center gap-4 border-t border-line p-7 md:border-l md:border-t-0">
                          <button
                            ...
                            className={`inline-flex items-center justify-center gap-3 px-6 py-3.5 text-sm transition-transform duration-300 hover:-translate-y-1 disabled:opacity-60 ${
                              e.is_rsvped ? "border border-forest text-forest" : "bg-clay text-white"
                            }`}
                          >
                            {e.is_rsvped ? (
                              <>
                          ...
                          {!user && <p className="text-center text-xs text-ink/45">Sign in required to RSVP</p>}
                          {e.is_rsvped && <p className="text-center text-xs text-ink/45">Tap again to cancel</p>}
                        </div>
                      </article>
                    </StaggerItem>
                  );
          ...
          {filtered.length === 0 && (
            <p className="py-20 text-center text-sm text-ink/50" data-testid="events-empty">
