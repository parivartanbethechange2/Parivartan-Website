...
const StackCard = ({ c, i, total, onFund }) => {
  ...
  const { scrollYProgress } = useScroll({
    target: ref,
    offset: ["start 96px", "end 300px"],
  });
  const scale = useTransform(scrollYProgress, [0, 1], [1, 0.93]);
  const opacity = useTransform(scrollYProgress, [0, 1], [1, 0.35]);
  const goal = Number(c?.goal) || 0;
  const raised = Number(c?.raised) || 0;
  const pct = goal > 0 ? Math.round((raised / goal) * 100) : 100;
  const code = c?.code || "campaign";
    ...
    >
      ...
      >
        <div className="relative h-56 overflow-hidden lg:h-auto">
          ...
          />
          <span className="absolute left-5 top-5 bg-sand/90 px-3 py-1.5 overline text-forest">
            {code}
          </span>
        </div>
        <div className="border border-line p-8 md:p-12">
          ...
          <h2 className="mt-6 serif text-3xl leading-tight tracking-tight md:text-4xl">
          ...
          <div className="mt-8 flex flex-wrap gap-x-8 gap-y-3 text-xs text-ink/55">
            ...
            {(Number(c?.trees_target) || 0) > 0 && (
              <span className="flex items-center gap-2">
                <Target size={13} />
                {fmt(Number(c?.trees_planted) || 0)} /{" "}
                {fmt(Number(c?.trees_target) || 0)} trees
              </span>
            )}
          </div>
          {goal > 0 && (
            <div className="mt-9">
              <div className="flex items-end justify-between">
                <p className="text-sm text-ink/60">
                  <span className="serif text-2xl text-forest">
                    {inr(raised)}
                  </span>{" "}
                  raised of {inr(goal)}
                </p>
          ...
          <div className="mt-9 flex flex-wrap gap-4">
            <button
              onClick={() => onFund(c)}
              data-testid={`fund-btn-${String(code).toLowerCase()}`}
            ...
            >
              {status === "active" ? "Fund this cause" : "Completed"}
            </button>
          </div>
        </div>
      </motion.article>
...
const Budget = () => (
  <section className="relative overflow-hidden bg-forest text-sand grain">
    <div className="mx-auto max-w-[1400px] px-6 py-24 md:py-32">
      <Reveal>
        <p className="overline text-sage/70">
          UGEPI budget • {UGEPI.budget}
        </p>
        ...
        {UGEPI.budgetBreakdown.map((b) => (
          <StaggerItem key={b.year}>
            <div className="border-t border-sand/25 pt-6">
              ...
              <p className="mt-5 serif text-5xl tracking-tight">
                {b.amount}
              </p>
        ...
        <div className="mx-auto max-w-[1400px] px-6 py-24 md:py-32">
          <Reveal>
            <p className="overline text-clay">
              Now running • {active.length} campaigns
            </p>
          </Reveal>
          <div className="mt-12">
            {active.map((c, i) => (
      ...
      <section className="bg-sand">
        <div className="mx-auto max-w-[1400px] px-6 py-24 md:py-32">
          ...
          <Stagger className="mt-14 grid gap-10 md:grid-cols-2">
            {past.map((c, i) => {
              const code = c?.code || `past-${i}`;
              return (
                <StaggerItem key={c?.id || code}>
                  <div
                    data-testid={`past-project-${String(
                      code
                    ).toLowerCase()}`}
                  >
                    <div className="aspect-[16/10] overflow-hidden">
                      <img
                        src={c?.cover || ""}
                        alt={c?.title || "Completed campaign"}
                        className="h-full w-full object-cover"
                      />
                    </div>
                    <p className="overline mt-6 text-ink/45">
                      {c?.period || ""}
                    </p>
                    <h3 className="mt-3 serif text-2xl tracking-tight md:text-3xl">
